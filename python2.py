...
def create_to_json(currency_code, date, rate):
    ...
    if currency_code in ('USD', 'EUR', 'PLN', 'GEL'):
        ...
        def prepare_message_for_json_file(currency_rate1, currency_rate2):
            my_memory = [
                {
                    'currency' currency_code[0]['cc']
                'current rate' currency_rate1[0]['cc']
                'rate of the last year' currency_rate2[0]['cc']
                'difference' currency_rate1 - currency_rate2[0]['rate']
                'date1': 20220313
                'date2': 20210313
                'time': time
                }
...
print(prepare_message_for_json_file(currency_rate1, currency_rate2))
